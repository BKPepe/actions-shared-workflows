...
# Quote a value as Markdown inline code. The fence is one backtick longer
# than the longest backtick run in the value, and the spaces inside it let
# the value start or end with a backtick. A carriage return would be a line
# ending in Markdown and end the span, so it is dropped.
code_span() {
	local text="${1//$'\r'/}"
	local fence='`'
